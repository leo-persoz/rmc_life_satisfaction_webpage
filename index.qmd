...
# Create table
differences_tables <- cantril %>%
  filter(Year %in% c(2011, 2023)) %>% 
  pivot_wider(names_from = Year,
              values_from = "Cantril ladder score") %>% 
  select(!Code) %>%
  mutate(abs_change = round(`2023` - `2011`, 2),
         rel_change = round((`2023` - `2011`) / `2011`, 2),
         rel_change = percent(rel_change))

reactable(differences_tables)
```
...
# Create line chart
filtered_cantril <- cantril %>%  
  filter(Country %in% c("Switzerland", "Afghanistan", "Ghana", "Kenya", "Colombia", "Brazil")) 
 
ggplot(filtered_cantril, aes(x = Year, y = `Cantril ladder score`, color = Country)) +
  geom_line() +
  scale_y_continuous(limits = c(0, 10)) +
  labs(title = "Cantril score for selected countries between 2011 and 2023",
       x = "Years",
